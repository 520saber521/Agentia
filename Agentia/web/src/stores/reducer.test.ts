...
import type { Message, ServerEvent, Task } from "../types";
...
describe("reduceEvent — history", () => {
  ...
  it("history 会合并同一工具的 running/done 卡片", () => {
    const state = withConv("conv_demo");
    const msg = makeMessage({
      sender_id: "agent_mock",
      sender_type: "agent",
      toolCalls: [
        { toolName: "web_search", status: "running", step: 1 },
        { toolName: "web_search", status: "done", resultSummary: "搜索结果", step: 2 },
      ],
    });
    const { next } = reduceEvent(state, {
      type: "history",
  ...
  it("history 会加载任务状态（刷新后首次加载）", () => {
    const state = withConv("conv_demo");
    const task = makeTask({ id: "task_parent", status: "running" });
    const sub = makeTask({ id: "task_sub", parent_task_id: "task_parent", status: "running", message_id: "msg_agent" });
    const { next } = reduceEvent(state, {
      type: "history",
      ts: 0,
      conversation_id: "conv_demo",
    ...
    expect(next.tasks.task_parent?.status).toBe("running");
    expect(next.tasks.task_sub?.status).toBe("running");
  });
...
describe("reduceEvent — message_created", () => {
  ...
  it("当前会话：追加用户消息，不改 streaming 标志", () => {
    ...
    expect(next.agentTyping).toEqual({});
    expect(effects).toEqual([]);
  });

  it("当前会话：agent 消息进来时 streamingMessageIds 追加并清掉 typing", () => {
    const state: ChatSlice = { ...withConv("conv_demo"), agentTyping: { agent_mock: true } };
    const msg = makeMessage({
      id: "msg_agent",
      sender_id: "agent_mock",
      sender_type: "agent",
...
describe("reduceEvent — agent_typing & stream_chunk", () => {
  it("agent_typing 命中当前会话时翻开 typing 标志", () => {
    ...
    expect(next.agentTyping).toEqual({ agent_mock: true });
  });
  ...
  it("stream_chunk 完全重复片段时不重复追加", () => {
    ...
    expect((next.messages[0].content as { text: string }).text).toBe("明确明确");
  });
...
describe("reduceEvent — task_update", () => {
  it("task_update 会保存 message_id 以便消息完成时兜底同步任务状态", () => {
    const task = makeTask({ id: "task_child", parent_task_id: "task_parent" });
    const { next } = reduceEvent(withConv("conv_demo"), {
      type: "task_update",
      ts: 0,
      conversation_id: "conv_demo",
      task,
    ...
    expect(next.tasks.task_child.message_id).toBe("msg_agent");
  });
});
...
describe("reduceEvent — message_done / cancelled / error", () => {
  ...
  it("message_done 会兜底把关联 message_id 的 running 子任务标记完成，并在全部子任务完成时完成父任务", () => {
    const parent = makeTask({ id: "task_parent", parent_task_id: null, status: "running" });
    const doneChild = makeTask({ id: "task_done", parent_task_id: "task_parent", status: "done", progress_pct: 100 });
    const runningChild = makeTask({
      id: "task_running",
    ...
    const { next } = reduceEvent(state, {
      ...
      message_id: "msg_agent",
      conversation_id: "conv_demo",
      final_content: { type: "text", text: "final" },
    });
    expect(next.tasks.task_running.status).toBe("done");
    expect(next.tasks.task_running.progress_pct).toBe(100);
    expect(next.tasks.task_parent.status).toBe("done");
  });

  it("message_cancelled 不会把关联任务误标记为完成", () => {
    const task = makeTask({ id: "task_running", status: "running", message_id: "msg_agent" });
    const placeholder = makeMessage({ id: "msg_agent", sender_type: "agent" });
    const state: ChatSlice = {
      ...withConv("conv_demo"),
      messages: [placeholder],
      streamingMessageIds: ["msg_agent"],
      tasks: { task_running: task },
    };
    const { next } = reduceEvent(state, {
    ...
    expect(next.tasks.task_running.status).toBe("running");
  });
