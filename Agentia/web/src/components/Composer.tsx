...
export function Composer() {
  ...
  const cancellableStreamingCount = useChatStore(
    (s) => s.streamingMessageIds.filter((messageId) => {
      const message = s.messages.find((item) => item.id === messageId);
      return message?.sender_id !== "agent_orchestrator";
    }).length,
  );
  ...
  useEffect(() => {
    function handler(e: Event) {
      ...
      setPendingCode({ code: detail.code, title: detail.title, senderId: detail.senderId, senderName: detail.senderName, messageId: detail.messageId, artifactId: detail.artifactId });
      if (detail.senderId || detail.senderName) {
        const senderAgent = detail.senderId
          ? agents.get(detail.senderId)
          : detail.senderName
            ? Array.from(agents.values()).find((a) => a.name === detail.senderName)
            : undefined;
        if (senderAgent) {
          setMentions((prev) => new Map(prev).set(senderAgent.id, senderAgent));
          setText((prev) => `@${senderAgent.name} ${prev}`);
        }
      }
  ...
  function doSend() {
    ...
    if (pendingCode) {
      const sourceInfo = pendingCode.senderName ? `（来源：${pendingCode.senderName}）` : "";
      fullText = `请修改以下代码${sourceInfo}（${pendingCode.title}）：\n\`\`\`\n${pendingCode.code}\n\`\`\`\n\n${fullText}`;
    }
  ...
  function sendDeployCommand() {
    if (!currentConvId) return;
    const orchestrator = isGroup ? memberAgents.find((a) => a.id === "agent_orchestrator") : undefined;
    send("部署", orchestrator ? [orchestrator.id] : undefined);
  }
      ...
      <div className="flex items-end gap-2">
        ...
        {streaming ? (
          <button
            type="button"
            onClick={cancel}
            disabled={cancellableStreamingCount === 0}
            className="shrink-0 rounded-md bg-rose-700 px-4 py-2 text-sm text-white transition hover:bg-rose-600 disabled:cursor-not-allowed disabled:bg-border disabled:text-muted"
            title={cancellableStreamingCount > 1 ? `取消 ${cancellableStreamingCount} 条普通 Agent 流式回复` : cancellableStreamingCount === 1 ? "取消普通 Agent 回复" : "Orchestrator 正在等待所有子任务完成，不能提前取消"}
          >
            {cancellableStreamingCount > 1 ? `取消 (${cancellableStreamingCount})` : "取消"}
          </button>
        ) : (
          <>
            <button
              type="button"
              onClick={sendDeployCommand}
              disabled={!currentConvId}
              ...
              title="部署前端 Agent 最新 HTML 产物到 Netlify"
            >
              部署
            </button>
            <button
