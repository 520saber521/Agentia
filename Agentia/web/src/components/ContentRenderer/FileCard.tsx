...
export function FileCard({ fileName, mimeType, fileSize, downloadUrl, fillHeight }: Props) {
  ...
  const previewMaxHeightClass = fillHeight ? "min-h-0 flex-1" : "max-h-[420px]";
  const codePreviewMaxHeight = fillHeight ? 9999 : 420;

  return (
    <div className={`rounded-xl border border-border bg-bg overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.14)] group ${fillHeight ? "flex h-full min-h-0 flex-col my-0" : "my-2"}`}>
      {/* Image preview */}
      ...
      {isText(mimeType) && (
        <div className={`border-b border-border ${fillHeight ? "min-h-0 flex-1" : ""}`}>
          {textLoading && (
              ...
              {(!isMarkdown(mimeType) || viewMode === "raw") && (
                <div className={`${previewMaxHeightClass} overflow-auto`}>
                  {isCodeLike(mimeType) || isMarkdown(mimeType) ? (
                    <CodeBlock code={displayText} language={lang} title={fileName} mini maxHeight={codePreviewMaxHeight} />
                  ) : (
