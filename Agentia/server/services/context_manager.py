...
def clear_context_cache(conversation_id: str | None = None) -> None:
...
class ContextManager:
    ...
    async def load(
        ...
        *,
        ...
        self._pinned_truncated = 0
        self._cache_signature = "|".join(
            str(m.get("id") or "") + ":" + str(m.get("created_at") or "") + ":" + str(int(bool(m.get("pinned"))))
            for m in self.history[-80:]
        )
    ...
    def _trim_pinned_context_to_budget(self) -> None:
        if not self.project_context:
            return
        budget = max(512, int(self.max_tokens * 0.35))
        selected: list[dict[str, Any]] = []
        used = 0
        for msg in sorted(self.project_context, key=lambda m: int(m.get("created_at") or 0), reverse=True):
            tok = estimate_tokens(str(msg.get("content") or ""))
            if selected and used + tok > budget:
                continue
            selected.append(msg)
            used += tok
        selected_ids = {id(m) for m in selected}
        self._pinned_truncated = max(0, len(self.project_context) - len(selected))
        self.project_context = sorted(selected, key=lambda m: int(m.get("created_at") or 0))
        if self._pinned_truncated:
            self.project_context.insert(0, {
                "role": "system",
                "content": f"[Pinned Context Notice] 由于长期上下文超出预算，已优先保留最新 {len(selected)} 条 pinned 消息，截断较早的 {self._pinned_truncated} 条。",
                "pinned": False,
                "artifact_id": None,
                "sender_id": "system",
                "created_at": 0,
            })
        self.history = [m for m in self.history if not m.get("pinned") or id(m) in selected_ids]

    def _format_pinned_context_message(self, msg: dict[str, Any]) -> str:
        if msg.get("sender_id") == "system":
            return str(msg.get("content") or "")
        role = "用户" if msg.get("sender_type") == "user" or msg.get("role") == "user" else "Agent"
        marker = str(msg.get("id") or "")
        artifact = f"\nartifact_id: {msg.get('artifact_id')}" if msg.get("artifact_id") else ""
        return f"[Pinned Long-Term Context | {role} | message_id: {marker}]\n{msg.get('content') or ''}{artifact}"

    def _prune(self) -> None:
        self._trim_pinned_context_to_budget()
    ...
    def build(self) -> list[dict[str, Any]]:
        ...
        cache_key = f"{self.conversation_id}:{self._pinned_count}:{len(self.tool_context)}:{self._pinned_truncated}:{hash(self._cache_signature)}"
        ...
        for msg in self.project_context:
            messages.append({"role": "system", "content": self._format_pinned_context_message(msg)})
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "pinned_truncated": self._pinned_truncated,
