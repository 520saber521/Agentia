...
import { describeApiError, type SaveAgentInput } from "../api/client";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: SaveAgentInput = {
        ...
        ...(lockedPrompt ? {} : { system_prompt: systemPrompt.trim() }),
      };
      if (apiKey.trim()) {
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted">
              Configure provider, encrypted API key, model, and role prompt.
            </p>
          </div>
          <button
            type="button"
