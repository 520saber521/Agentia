...
export function ConversationListPanel() {
  ...
  return (
    <aside className="flex min-h-0 w-full shrink-0 flex-col overflow-hidden border-r border-border bg-panel">
      ...
      <div className="border-b border-border p-3">
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-2xs font-semibold uppercase text-muted">Agent contacts</h3>
          <button
            type="button"
            ...
            <div key={agent.id} className="group rounded-md border border-border p-2 transition hover:border-accent/60 hover:bg-accent/10">
              <button type="button" onClick={() => void startAgentChat(agent.id)} className="w-full text-left">
                <div className="flex items-center gap-2">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent/15 text-accent">
                    {agent.avatar ? (
                      <span className="text-xs font-semibold">{agent.avatar}</span>
                    ) : (
              ...
              <div className="mt-2 flex items-center justify-between gap-2">
                  ...
                  {agent.capabilities.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {agent.capabilities.slice(0, 4).map((cap) => (
                        <span key={cap} className={`rounded border px-1.5 py-0.5 text-2xs ${capColor(cap)}`}>
                          {cap}
                        </span>
                      ))}
                      {agent.capabilities.length > 4 && (
                        <span className="rounded border border-border bg-bg px-1.5 py-0.5 text-2xs text-muted">
                          +{agent.capabilities.length - 4}
                        </span>
                      )}
                    </div>
                ...
                <div className="flex shrink-0 items-center gap-1">
                  {agent.is_system && (
                    <span className="rounded border border-accent/30 bg-accent/5 px-1 py-0.5 text-2xs text-accent/70">system</span>
                  )}
                  <button
                    type="button"
                    onClick={(e) => {
                    ...
                    className="rounded border border-accent/50 bg-accent/10 px-2 py-0.5 text-2xs text-accent transition hover:bg-accent/20 hover:border-accent"
                  ...
                  <button
                    ...
                    className="rounded border border-border px-1.5 py-0.5 text-2xs text-muted transition hover:border-accent/60 hover:text-fg"
                    title="View prompt"
                  >
                    Prompt
                  </button>
              ...
              {promptAgentId === agent.id && (
                <div className="mt-2 rounded border border-border bg-bg/80">
                  {promptLoading ? (
                    <div className="px-2 py-2 text-2xs italic text-muted">Loading prompt...</div>
                  ) : promptText ? (
                    <pre className="max-h-32 overflow-y-auto whitespace-pre-wrap px-2 py-2 font-mono text-2xs leading-relaxed text-muted">
                      {promptText.slice(0, 800)}
                      {promptText.length > 800 && <span className="text-accent/60"> ... (truncated)</span>}
                    </pre>
                  ) : (
                    <div className="px-2 py-2 text-2xs italic text-muted">No prompt available.</div>
                  )}
                </div>
              )}
            </div>
              ...
              <div className={`rounded-md border transition ${active ? "border-accent bg-accent/10" : "border-border hover:border-accent/60"}`}>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        {c.pinned && <span className="text-2xs text-accent">置顶</span>}
                    ...
                    <span className="shrink-0 text-2xs text-muted">{formatTime(c.updated_at)}</span>
                  </div>
                ...
                <div className="flex items-center gap-1 border-t border-border px-2 py-1">
                  <button
                    ...
                    className="rounded px-2 py-1 text-2xs text-muted transition hover:bg-bg hover:text-fg"
                  >
                    {c.pinned ? "取消置顶" : "置顶"}
                  </button>
                  <button
