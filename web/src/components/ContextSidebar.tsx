          ...
          <section key={panel.id} className="mb-2 overflow-hidden rounded-md border border-border bg-bg/70">
            ...
            {open[panel.id] && panel.id === "context" && (
              <div className="space-y-2 p-3 text-xs">
                ...
                {contextStats?.pinnedTruncated ? (
                  <div className="rounded border border-amber-500/25 bg-amber-500/10 p-2 text-[10px] leading-relaxed text-amber-200">
                    长期上下文已按最新 pinned 优先保留，较早 {contextStats.pinnedTruncated} 条未注入本轮调用。
                  </div>
                ) : null}
                <div className="text-[10px] leading-relaxed text-muted">
                  Pinned messages are injected before recent history as long-term Agent context.
                </div>
              </div>
            )}
