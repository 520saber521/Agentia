...
from services.message import append_message_tool_call
...
from services.deploy import deploy_frontend_html_to_netlify, is_deploy_request
...
from ws import Connection, event, hub, in_flight_registry
...
async def _emit_conversation(
    ...
    await conn.send(evt)
    await hub.broadcast_conversation(conversation_id, evt, exclude=conn)
...
def _looks_complex_task(user_text: str) -> bool:
    ...
    keywords = [
        ...
        "orchestrator", "协调", "多 agent", "multi-agent", "部署", "deploy", "netlify",
    ]
...
async def resolve_targets(
    ...
    if is_deploy_request(user_text) and ORCHESTRATOR_AGENT_ID in agent_member_ids:
        return [ORCHESTRATOR_AGENT_ID], None
...
async def _run_netlify_deploy_request(
    conn: Connection,
    conversation_id: str,
    user_text: str,
    target_agent_ids: list[str],
) -> None:
    sender_id = target_agent_ids[0] if target_agent_ids else ORCHESTRATOR_AGENT_ID
    ...
    pending_content = {
        ...
        "status": "pending",
        "title": "Netlify 部署准备中",
        "summary": "正在查找当前对话中前端 Agent 生成的最新 HTML / preview 产物。",
        "progress": 10,
    }
    ...
    async with Session() as s:
        ...
        deploy_msg = await create_message(
            s,
            conversation_id=conversation_id,
            sender_id=sender_id,
            sender_type="agent",
    ...
    await _emit_conversation(conn, conversation_id, event("message_created", message=user_msg_dict))
    await _emit_conversation(conn, conversation_id, event("message_created", message=deploy_msg_dict))
    await _emit_conversation(conn, conversation_id, event(
        "deploy_status",
        conversation_id=conversation_id,
        deploy_id=deploy_id,
        status="pending",
    ...
    building_content = {
        ...
        "status": "deploying",
        "title": "正在部署到 Netlify",
        "summary": "已定位部署请求，正在上传前端 HTML 静态站点。",
        "progress": 55,
    }
    async with Session() as s:
        await update_message_content(s, deploy_msg_dict["id"], building_content)
    await _emit_conversation(conn, conversation_id, event(
        "deploy_status",
        conversation_id=conversation_id,
        deploy_id=deploy_id,
    ...
    async with Session() as s:
        result = await deploy_frontend_html_to_netlify(s, conversation_id)

    final_status = "deployed" if result.get("ok") else "failed"
    final_content = {
        "type": "deploy_status",
        "deploy_id": deploy_id,
        "status": final_status,
        "title": "Netlify 部署完成" if result.get("ok") else "Netlify 部署失败",
        "summary": str(result.get("summary") or "部署流程已结束。"),
        "progress": 100,
    }
    if result.get("url"):
        final_content["url"] = str(result["url"])

    async with Session() as s:
        updated = await update_message_content(s, deploy_msg_dict["id"], final_content)
        final_msg = message_to_dict(updated) if updated is not None else {**deploy_msg_dict, "content": final_content}
...
async def handle(conn: Connection, evt: dict[str, Any]) -> None:
    ...
    if is_deploy_request(user_text):
        await _run_netlify_deploy_request(conn, conversation_id, user_text, target_agent_ids)
        return
    ...
    for aid, mid, _mdict in agent_msgs:
        ...
        await in_flight_registry.add(mid, task)
        task.add_done_callback(
            lambda _t, _m=mid: conn.in_flight.pop(_m, None)
        )
        task.add_done_callback(
            lambda _t, _m=mid: asyncio.create_task(in_flight_registry.pop(_m))
        )
...
async def _load_pinned_context_messages(conversation_id: str, limit: int = 20) -> list[dict[str, str]]:
    Session = get_sessionmaker()
    async with Session() as s:
        rows = (
            await s.scalars(
    ...
    for m in reversed(rows):
        ...
        if not str(text).strip():
            continue
        role = "用户" if m.sender_type == "user" else "Agent"
        messages.append({"role": "system", "content": f"[Pinned Long-Term Context | {role} | message_id: {m.id}]\n{text}"})
    return messages
...
async def run_agent_reply(
    ...
    if builtin_loop:
        pinned_messages = await _load_pinned_context_messages(conversation_id)
        messages = [*pinned_messages, {"role": "user", "content": user_text}]
        ...
        await conn.send(
            event(
                ...
                pinned_truncated=ctx.get("pinned_truncated", 0),
    ...
    try:
        async for chunk in chunk_source:
            ...
            if ctype == "text":
                ...
                async with Session() as s:
                    await append_message_tool_call(
                        s,
                        ai_msg_id,
                        tool_name=tool_name,
                        status="running",
