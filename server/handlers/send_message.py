...
async def handle(conn: Connection, evt: dict[str, Any]) -> None:
    ...
    # Await router health check (started in parallel with DB work above)
...
async def run_agent_reply(
    ...
    # Build conversation context using ContextManager (4-layer architecture)
    ...
    # 发送上下文统计信息（每次 Agent 请求都展示已携带多少上下文）
    ctx = cm.summary()
    await conn.send(
        event(
            "context_info",
    ...
    registry = get_tool_registry(project_root="D:/Agentia/Agentia")
