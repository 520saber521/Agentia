...
class InFlightRegistry:
    ...
    async def add(self, message_id: str, task: asyncio.Task[Any]) -> None:
        async with self._lock:
            self._tasks[message_id] = task

    async def pop(self, message_id: str) -> asyncio.Task[Any] | None:
        async with self._lock:
            return self._tasks.pop(message_id, None)

    async def get(self, message_id: str) -> asyncio.Task[Any] | None:
        async with self._lock:
...
class Connection:
    ...
    def __init__(self, ws: WebSocket) -> None:
        ...
        self.outbound: asyncio.Queue[dict[str, Any] | None] = asyncio.Queue(maxsize=4096)
    ...
    async def close(self, *, cancel_in_flight: bool = False) -> None:
        self._closed = True
        if cancel_in_flight:
