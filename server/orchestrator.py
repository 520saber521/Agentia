...
async def _emit_parent_task_update(
    conn: Connection,
    conversation_id: str,
    parent: Any,
    *,
    ...
    await conn.send(event(
        ...
        progress=task_dict.get("progress_pct", 0),
        message_id=task_dict.get("originating_message_id"),
        action=action,
    ))
...
async def handle_orchestrator_mention(
    ...
    async with Session() as s:
        ...
        for m in reversed(pinned_rows):
            ...
            if text.strip():
                ...
                pinned_context.append(f"[Pinned Long-Term Context | {role} | message_id: {m.id}]\n{text}")
    ...
    await _emit_parent_task_update(conn, conversation_id, parent)
    ...
    for st, _agent_name, _aid, _is, _dep in subtask_records:
        async with Session() as s:
            running_st = await update_task_status(s, st.id, "running")
        ...
        task_dict = task_to_dict(running_st or st)
        task_dict["depends_on"] = _dep
        await conn.send(event("message_created", message=msg_dict))
        await conn.send(event("agent_typing", agent_id=_aid, conversation_id=conversation_id))
        await conn.send(event(
            "task_update",
            conversation_id=conversation_id,
            task=task_dict,
            task_id=task_dict.get("parent_task_id") or task_dict["id"],
            subtask_id=task_dict["id"] if task_dict.get("parent_task_id") else None,
            status="running",
            progress=task_dict.get("progress_pct", 0),
            message_id=msg_id,
            action="created",
    ...
    immediate_dag = DAG()
    for st, _agent_name, _agent_id, _input_summary, _deps in subtask_records:
        immediate_dag.add_node(DAGNode(
            id=st.id,
            domain=st.domain or "",
            ...
            metadata={"task_record": st, "message_id": subtask_message_ids[st.id]},
        ))
    ...
    async def _dispatch_node(node: DAGNode) -> str:
        ...
        try:
            result_message_id = await _dispatch_subtask_with_retry(
                ...
                message_id=node.metadata["message_id"],
            )
            animation_bus.viz_event(
                conversation_id=conversation_id,
                kind="llm",
    ...
    executor = DAGExecutor(immediate_dag, _dispatch_node, max_concurrency=len(subtask_records))
    ...
    if all_done:
        ...
        if demo_mode:
            ...
            for st, agent_name, _aid, _is_, _dep in subtask_records:
                output_text = subtask_outputs.get(st.id, "")
                if output_text:
                    # Extract a concise summary: first meaningful line(s) or key content
                    lines = [l.strip() for l in output_text.split("\n") if l.strip()]
                    # Skip leaked tool_call blocks in summary
                    lines = [l for l in lines if not l.startswith("```")]
                    summary_lines = lines[:10]  # first ~10 non-empty lines
                    snippet = "\n".join(summary_lines)
                    if len(snippet) > 600:
                        snippet = snippet[:600] + "…"
                    summary_text += f"**{agent_name}（{st.domain or 'general'}）**\n{snippet}\n\n"
                else:
        ...
        summary_text = (
            ...
            else f"⚠️ **Task Partially Complete**\n\n"
        )
        summary_text += (
        ...
        for st, agent_name, _aid, _is_, _dep in subtask_records:
            if st.id in completed_ids:
                ...
                if output_text:
                    ...
                    summary_text += f"- ✅ {agent_name}：{st.title[:60]}\n"
            else:
                summary_text += f"- ❌ {agent_name}：{st.title[:60]}\n"
    ...
    await _emit_parent_task_update(
        conn,
        conversation_id,
        parent,
        action="completed" if parent is not None and parent.status == "done" else "status_changed",
    )
...
async def _dispatch_subtask_with_result(
    ...
    """Dispatch a subtask to an agent and stream into its message bubble.
    ...
    try:
        async for chunk in chunk_source:
            ...
            if ctype == "text":
                ...
                obs_name = str(chunk.get("name") or chunk.get("tool_name") or chunk.get("tool") or "tool")
                obs_result = chunk.get("result")
                if obs_result is None:
                    obs_result = chunk.get("observation", chunk.get("content", ""))
                obs_text = obs_result if isinstance(obs_result, str) else json.dumps(obs_result, ensure_ascii=False)
                async with Session() as s:
                    await append_message_tool_call(
                        s,
                        msg_id,
