...
def message_to_dict(m: Message) -> dict[str, Any]:
    ...
    if isinstance(content, dict) and isinstance(content.get("toolCalls"), list):
        msg["toolCalls"] = content["toolCalls"]
    return msg
...
async def update_message_content(
    ...
    if m is None:
        return None
    existing = _safe_loads(m.content)
    if isinstance(existing, dict) and "toolCalls" in existing and "toolCalls" not in content:
        content = {**content, "toolCalls": existing["toolCalls"]}
    normalized_content = validate_content(content)
...
async def append_message_tool_call(
    s: AsyncSession,
    message_id: str,
    *,
    tool_name: str,
    ...
    if content.get("type") != "text" and "text" not in content:
        content = {**content, "text": ""}
    calls = content.get("toolCalls")
    if not isinstance(calls, list):
        calls = []
    next_call: dict[str, Any] = {"toolName": tool_name, "status": status}
    if result_summary is not None:
        next_call["resultSummary"] = result_summary

    tool_key = tool_name.strip().lower()
    found_idx = -1
    for idx, call in enumerate(calls):
        if isinstance(call, dict) and str(call.get("toolName") or "").strip().lower() == tool_key:
            found_idx = idx
            break

    if found_idx >= 0:
        existing_call = calls[found_idx] if isinstance(calls[found_idx], dict) else {}
        calls[found_idx] = {**existing_call, **next_call}
    else:
        next_call["step"] = len(calls) + 1
